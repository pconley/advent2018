...
def parse(d):
    ...
    return dt, words[1], words[2]

events = []

for d in data:
    events.append(parse(d))
    print(events[-1])
...
for e in events:
    ...
    if action == "Guard":
        ...
        state = "W"
        print state,guard,dt
        slept = dt.minute-start
        print guard, "slept", slept
        guards[guard] = guards.get(guard,0) + slept
    elif action == "falls" :
        state = "S"
        start = dt.minute
        print state,guard,dt
...
for e in events:
    ...
    if action == "Guard":
        ...
        state = "W"
        stop = dt.minute
        print guard, "slept", start, stop
        if not guard in profiles:
            profiles[guard] = [0 for i in range(0,60)]
        prof = profiles.get(guard)
...
for p in profiles:
    print p, profiles[p]
    if max(profiles[p]) > max_zz:
        max_zz = max(profiles[p])
        max_id = p

for i in range(0,60):
    print i, profiles[max_id][i]
    if profiles[max_id][i] == max_zz :
        save = i

print max_id, max_zz, save
