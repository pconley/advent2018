...
canvas = [ [ '_' for i in range(0,size) ] for i in range(0,size) ]
previous_canvas = []
...
def cprint(canvas):
    count = 0
    for row in canvas :
        print "{0:5d} : ".format(count)+''.join(str(x) for x in row)
        count += 1
...
for i in range(size):
    for j in range(size):
        dists = [mdist(x,y,i,j) for (y,x) in data]

        # Part 1
        small = min(dists)
        count = sum([1 for d in dists if d == small ])
        if count == 1 :
            # there was only one closest point
            index = dists.index(small)
            canvas[i][j] = letters[index]
        else :
            # there was a tie for closness
            canvas[i][j] = '.'
